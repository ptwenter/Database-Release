...
/* THIS SCRIPT PERFORMS THE FOLLOWING:
	...
	- ASSOCIATES THE USER LUCITY_USER TO THE GBAGROUP ROLE IF THE ROLE EXISTS
	...
	- ASSOCIATES THE USER LUCITY_GATEWAY TO THE GBAMS_GATEWAY ROLE IF THE ROLE EXISTS
...
BEGIN
		...
		SET @SQL = 'USE ' + @DATABASE + '
			   ...
			   EXEC sp_addrolemember ''GBAGroup'', ''LUCITY_USER'''
		EXEC (@SQL)
		...
		BEGIN
			SET @SQL = 'USE ' + @DATABASE + '
			   ...
			   IF EXISTS (SELECT * FROM dbo.sysusers WHERE name = N''GBAMS_GATEWAY'' AND ISSQLROLE = 1)
			   EXEC sp_addrolemember ''GBAMS_GATEWAY'', ''LUCITY_GATEWAY'''
			EXEC (@SQL)
